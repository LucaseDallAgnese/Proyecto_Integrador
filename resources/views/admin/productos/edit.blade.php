        ...
        <form action="{{ route('admin.productos.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="number" name="discount" id="discount" class="shadow appearance-none border @error('discount') border-red-500 @enderror rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" step="0.01" value="{{ old('discount', $product->discount ?? 0) }}" min="0" max="100">
                    @error('discount')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
